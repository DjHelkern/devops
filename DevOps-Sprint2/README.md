...
Команда: 
```
docker login -u djhelkern
```

Для того, чтобы доставить приложение на сервер испольуется Gitlab CI/CD.
Ссылка на созданный репозиторий: https://gitlab.com/djhelkern/devops
...
docker_build:
  ...
  script:
    - export
    - tag='$TAG'
    - docker build -t djhelkern/sprint2:$TAG .
    - docker login -u "$REG_USER" -p "$REG_PASSWORD"
    - docker push djhelkern/sprint2:$TAG
